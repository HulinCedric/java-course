...
//
// Classe MaillagePanneauG - Maillage generique d'un panneau, supportant toute
//                           homothetie dynamique de la taille de ce dernier
//
// Edition A    : Cours_10
//    + Version 1.0.0	: la maille elementaire ne peut etre qu'un rectangle
//    + Version 1.1.0   : ajout des methodes obtenirDesignation (classe 
//                        MaillagePanneauG et classe interne Maille)
...
// ------                                     *** Premier constructeur normal
   ...
   public MaillagePanneauG (JPanel hamecon, Color couleurTrait, 
                                            Color couleurFond,
                                            int nbLignes, 
                                            int nbColonnes) {
      ...
      for (int i= 1; i<=nbLignes; i++) 
         for (int j= 1; j<=nbColonnes; j++) ajouterMaille(i, j); 
   }
...
// ------                                       *** Methode obtenirDesignation
   
   public Dimension obtenirDesignation (int x, int y) {
      ...
      for (int i= 0; i<nbLignes; i++) {
         ...
         for (int j= 0; j<nbColonnes; j++) {
            ...
            //
            designation= mailleCourante.obtenirDesignation(x, y);
            if (designation) return new Dimension(i+1, j+1);
         } 
      }
   ...
   // ------                                    *** Methode obtenirDesignation
      ...
      public boolean obtenirDesignation (int x, int y) {
      	
         // Construire le polygone attache a la maille cible
         //
         Polygon polygone= obtenirPolygone();
      	
         // Determiner si le point cible (x, y) appartient a cette maille
         //	
         if(polygone.contains(x,y)) return true;
