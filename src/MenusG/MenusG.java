...
//    + Version 1.7.0 : ajout d'une methode de suppression d'ecouteurs sur un item
//    + Version 1.8.0 : ajout d'une methode d'ajout de raccourci sur un item
//
...
private Frame hameconAmont;
   ...
   MenuItem itemCourant=null; 
	   ...
	   // D使riser l'item
	   //
	   if (!activerItem(nomMenu, nomItem)) return false;
   ...
   // Enlever un ecouteur a un item						*** Methode enleverEcouteur
   //
   public boolean enleverEcouteur(String nomMenu, String nomItem, int rang) {
   MenuItem itemCourant= null;
   ActionListener ecouteur= null;
	   ...
	   // Verifier l'existence de l'ecouteur
	   // 
	   if (itemCourant.getActionListeners().length < rang)	return false;
	   ecouteur= itemCourant.getActionListeners()[rang];
	   if (ecouteur == null)	return false;
	   
	   // Enlever l'ecouteur sur l'item
	   //
	   itemCourant.removeActionListener(ecouteur);
      
	   // Griser l'item s'il n'a plus d'残outeurs
	   //
	   if(itemCourant.getActionListeners().length == 0)	itemCourant.setEnabled(false);
	   return true;
   }
   
   // Ajouter un raccourci a un item					*** Methode ajouterRaccourci
   //  
   public boolean ajouterRaccourci(String nomMenu, String nomItem, MenuShortcut  raccourci) {
   MenuItem itemCourant= null;
	   ...
	   // Attacher un raccourci a l'item courant
	   //
	   itemCourant.setShortcut(raccourci);
